...
import './interfaces/IHopPair.sol';
import './HopHRC20.sol';
import './Math.sol';
import './UQ112x112.sol';
import './HRC20.sol';
import './IHopFactory.sol';
import './IHopCallee.sol';

contract HopPair is IHopPair, HopHRC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (data.length > 0) IHopCallee(to).hopCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IHRC20(_token0).balanceOf(address(this));
        balance1 = IHRC20(_token1).balanceOf(address(this));
        }
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'Hop: K');
        }
